...
from model.aurora_finetune import NeuralTrackerV1
...
checkpoint_file = checkpoint_dir / "model_epoch_68.pt"

fig_dir = Path("figs") / model_name
...
test_data = TyphoonTrajectoryDataset(data_dir, 2022, 2022, lookback, horizon, with_era5=True)
test_dataloader = DataLoader(test_data, batch_size=batch_size, shuffle=False, drop_last=False,
                              num_workers=num_workers, pin_memory=True)

model = NeuralTrackerV1(obs_len=lookback, pred_len=horizon).to(device)
